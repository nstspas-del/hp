 ...
 * Logo.tsx — HP Тюнинг
 *
 * ВАЖНО: Логотип ВСЕГДА ведёт на главную основного домена (https://hptuning.ru/),
 * даже если пользователь находится на брендовом сабдомене (bmw.hptuning.ru и т.п.).
 *
 * Поведение специально единообразное:
 * - На главной (hptuning.ru) — клик = reload главной (использует <a>, чтобы не было
 *   мёртвого ховера в случае next/link на тот же путь).
 * - На сабдомене (bmw.hptuning.ru) — клик = переход на hptuning.ru/.
 *
 * Так юзер никогда не «залипает» внутри брендового сабдомена.
 */
...
const MAIN_HOME = 'https://hptuning.ru/';

export function Logo({ variant = 'full', className = '', onClick }: LogoProps) {
  return (
    <a
      href={MAIN_HOME}
      aria-label="HP Тюнинг — на главную hptuning.ru"
      onClick={(e) => {
        // Трекинг клика по логотипу
        if (typeof window !== 'undefined') {
          window.ym?.(108614238, 'reachGoal', 'logo_click_home');
        }
        onClick?.();
      }}
