...
export function generateMetadata({ params }: { params: { service: string } }): Metadata {
 ...
 description: service.shortDescription ?? `${service.name} в Санкт-Петербурге. Профессиональный автосервис. Запись онлайн.`,
 alternates: { canonical: `https://hptuning.ru/service/${params.service}` },
 openGraph: {
 ...
 url: `https://hptuning.ru/service/${params.service}`,
 images: [{ url: 'https://hptuning.ru/images/og/service.jpg', width: 1200, height: 630, alt: `${service.name} | HP Тюнинг СПб` }],
 },
 };
}

export default function AutoserviceDetailPage({ params }: { params: { service: string } }) {
 const service = getService(CAT, params.service);
 if (!service) notFound();
